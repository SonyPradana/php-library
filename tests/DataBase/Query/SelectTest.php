...
final class SelectTest extends \QueryStringTest
{
    ...
    /** @test */
    public function itCorrectSelectEscape(): void
    {
        $select = MyQuery::from('test', $this->PDO)
            ->select(['column 1', 'column_2 as column2', 'table1.column_3', 'table2.column_1 as column 1', 'table2.column_2 as column2b']);

        $this->assertEquals(
            'SELECT `column 1`, column_2 AS `column2`, `table1`.`column_3`, table2.column_1 AS `column 1`, table2.column_2 AS `column2b` FROM `test` ',
            $select->__toString(),
            'select statment must have 3 selected query'
        );
    }
